...
class thomas_player : public player {
	...
	int max_depth;
	public:
	thomas_player(int player_id, int depth = 8) :
		player_id(player_id), max_depth(depth)
	{
		time_t t;
		time(&t);
		srand(t);
	...
	int play(const playfield &field) {
		...
		int win = compute_good_moves(fld, good_moves, player_id, 0);
		if (win >= 0) return win;
			...
			set<int> &pmoves, int player_id, int depth)
	{
		...
		while (begin != end) {
			...
			fld.insert(pos, player_id);

			// filter out the bad moves, i.e., the opponent's win moves
			set<int> op_moves = fld.get_possible_moves();

			int win = compute_good_moves(fld, op_moves, other_player(player_id), depth+1);
			fld.remove(pos);
			if (win >= 0) {
	...
	{
		return std::make_shared<thomas_player>(id);
	}

	static register_player<thomas_player> p("thomas");
};
